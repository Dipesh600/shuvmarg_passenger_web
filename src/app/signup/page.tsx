...
export default function SignUpPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (step === "phone") {
      // Validate phone and move to OTP step
      if (formData.phone.length === 10) {
        setStep("otp");
      }
    } else {
      console.log("Signup Data (Phone & OTP):", formData);
      ...
      <div className="w-full h-full max-w-[1600px] bg-white rounded-[40px] overflow-hidden flex flex-col lg:flex-row shadow-2xl relative z-10">
        ...
        <div className="w-full lg:w-1/2 bg-white z-10 p-8 lg:p-12 flex flex-col justify-center relative">
          ...
          <div className="flex justify-end mb-8 lg:absolute lg:top-12 lg:right-12 lg:mb-0">
            <Link href="/" className="font-display font-bold text-[32px] tracking-tight text-[#0B3150] cursor-pointer">
          ...
          <div className="max-w-md w-full mx-auto flex flex-col justify-center">
            {step === "phone" ? (
              <>
                <h2 className="text-3xl lg:text-[40px] font-bold text-[#1A1A1A] mb-2 leading-tight">Create an Account</h2>
                <p className="text-[#5D4B3B] mb-8 text-lg">Welcome to ShuvMarg</p>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                  ...
                  {showReferral ? (
                    <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                      <label className="block text-[13px] font-bold text-[#1A1A1A] mb-2" htmlFor="referralCode">
                      ...
                      <input
                        ...
                        className="w-full h-[52px] px-4 rounded-xl border border-[#E2D6C6] outline-none text-[#1A1A1A] bg-white placeholder:text-[#A89886] focus:border-[#D94328] focus:ring-1 focus:ring-[#D94328] transition-colors uppercase"
                      />
                    </div>
                  ) : (
                    <button 
                      ...
                      className="text-[#D94328] text-[13px] font-bold text-left hover:underline w-fit"
                    >
                  ...
                  <div className="flex items-center gap-4 my-2">
                    <div className="h-px bg-[#E2D6C6] flex-1"></div>
                    <span className="text-[#A89886] text-xs font-medium uppercase tracking-wider">Or</span>
                    <div className="h-px bg-[#E2D6C6] flex-1"></div>
                  </div>

                  <div className="text-center text-[#5D4B3B] text-sm">
